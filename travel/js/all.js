...
const data_filter = (get_condition) => { //篩選資料、分頁
    ...
    for (let arr_index = 0; arr_index < ajax_data.result.records.length; arr_index++) {
        if (get_condition !== ajax_data.result.records[arr_index].Zone && get_condition !== '全部景點' && get_condition !== '請選擇行政區') { // get_condition ==='全部景點'  ==>資料全抓；get_condition=== option的value屬性(XX區)
            continue;
        } else {
...
const create_dom = (pickout_data, load_page_num_data) => {
    ...
    if (pickout_data.length === 0) { // || load_page_num_data <= 0
        ...
        for (let onepage_data_Index = 0; onepage_data_Index < pickout_data[load_page_num_data].length; onepage_data_Index++) {
            ...
            let card_ele = `
                    <section class=" card-picture position-relative" style="background-image:url(${ pickout_data[load_page_num_data][onepage_data_Index].Picture1})"  alt="${pickout_data[load_page_num_data][onepage_data_Index].Picdescribe1}">
                    ...
                    <img src="image/icons_phone.png"     alt="連絡電話">
                        ...
                        <ul class="flex justify-content-between align-items-end flex-wrap flex-grow-1">
                                ...
                                <li class=" flex align-items-center text-bold mb-13 flex-basis-100" >                        
                                    <p class="text-bold ml-12 text-oneLine">${ pickout_data[load_page_num_data][onepage_data_Index].Opentime}</p>
                                </li>                                
                                <li class=" flex align-items-center text-bold mb-13 flex-basis-100" >                                  
                                    <p class="text-bold ml-12 text-oneLine">${ pickout_data[load_page_num_data][onepage_data_Index].Add}</p>
                                </li>
                                ...
                                <a class="d-block text-bold ml-12 tex-origin" href="tel:${pickout_data[load_page_num_data][onepage_data_Index].Tel}">
                        ...
                        <p class="text-bold ml-12 text-oneLine" tittle="${  pickout_data[load_page_num_data][onepage_data_Index].Ticketinfo||'付費參觀'} ">${  pickout_data[load_page_num_data][onepage_data_Index].Ticketinfo||'付費參觀'}</p>
                    </li>
                                </div>    
                        </ul>
                    </section>
